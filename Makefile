...
help:
	@echo "DependIQ"
	@echo ""
	@echo "  make test       Run all tests (auto-starts Neo4j if needed)"
	@echo "  make lint       Check code for errors"
	@echo "  make format     Auto-format code"
	@echo "  make run        Start local server (handles all setup)"
	@echo "  make setup      First-time project setup (Postgres + Neo4j + env)"
	...
	@echo ""
	@echo "  make neo4j-start  Start Neo4j (Docker)"
	@echo "  make neo4j-stop   Stop Neo4j"
	@echo "  make neo4j-status Show Neo4j status"
	@echo ""
...
run: _check-uv _ensure-env _ensure-db
	@echo "Starting DependIQ on http://localhost:8000 ..."
	@$(UV) run uvicorn main:app --reload --host 0.0.0.0 --port 8000

setup: _check-uv _ensure-env _ensure-db _ensure-neo4j
	@echo ""
	@echo "Setup complete. All services running. Run 'make test' or 'make run'."
...
_ensure-neo4j: _check-docker
	@if docker compose exec -T neo4j-test cypher-shell -u neo4j -p dependiq_test_2026 "RETURN 1" >/dev/null 2>&1; then \
		true; \
	else \
...
_ensure-db:
	...
	@$(DB_SERVICE_START) >/dev/null 2>&1 || (echo "ERROR: Could not start PostgreSQL. Run: $(DB_SERVICE_START)"; exit 1)
	@sleep 0.5
	@if ! $(PSQL_CMD) -lqt 2>&1 | cut -d \| -f 1 | grep -qw dependiq; then \
...
db-start:
	@$(DB_SERVICE_START) && echo "PostgreSQL started" || \
		(echo "ERROR: Could not start PostgreSQL. Install with: brew install postgresql@14"; exit 1)

db-stop:
	@$(DB_SERVICE_STOP) && echo "PostgreSQL stopped" || \
		(echo "ERROR: Could not stop PostgreSQL."; exit 1)

db-reset: _ensure-db
	@echo "WARNING: This will delete all data and recreate the schema!"
	@read -p "Type 'yes' to continue: " confirm; \
	if [ "$$confirm" = "yes" ]; then \
		$(DROPDB_CMD) dependiq || (echo "ERROR: Could not drop database."; exit 1); \
		$(CREATEDB_CMD) dependiq || (echo "ERROR: Could not create database."; exit 1); \
...
db-status:
	@echo "Service:"
	@$(DB_SERVICE_STATUS) || echo "  Not running"
	@echo "Tables:"
	@$(PSQL_CMD) -d dependiq -c "\dt" 2>&1 || echo "  Not accessible"
...
neo4j-start: _check-docker
	@docker compose up -d neo4j-test
	@echo "Waiting for Neo4j to be healthy..."
	@until docker compose exec -T neo4j-test cypher-shell -u neo4j -p dependiq_test_2026 "RETURN 1" >/dev/null 2>&1; do sleep 1; done
	@echo "Neo4j ready at bolt://localhost:7687"

neo4j-stop: _check-docker
